...
let arquivo = fs.readFileSync("src/cotacao-do-bitcoin.csv", {
  encoding: "utf-8",
});
...
for (let l = 1; l < linhas.length; l++) {
  ...
  if (qtdLinhas == linhas.length - 2)
    celulas1 = ["31.12.2018", 3709.4, 3815.1, 3819.6, 3658.8];
...
const x = tf.tensor2d(X, [qtdLinhas, 4]);
const y = tf.tensor2d(Y);

const arrInput = [6984.8, 7190.0, 7518.9, 6802.6]; // 11.05.2019
// const arrInput = [7806.0, 6984.7, 8058.3, 6893.1]; // 12.05.2019
...
model
  ...
  .then(() => {
    ...
    console.log(`PREÇOS DAS COTAÇÕES:`);
    console.log(`Fechamento: $ ${Number(output[0]).toFixed(1)}`);
    console.log(`Abertura:   $ ${Number(output[1]).toFixed(1)}`);
    console.log(`Maxima:     $ ${Number(output[2]).toFixed(1)}`);
    console.log(`Minima:     $ ${Number(output[3]).toFixed(1)}`);
  });
