...
let arquivo = fs.readFileSync("src/cotacao-das-acoes-pn-da-petrobras.csv", {
  encoding: "utf-8",
});
...
for (let l = 1; l < linhas.length; l++) {
  ...
  if (qtdLinhas == linhas.length - 2)
    celulas1 = ["28.12.2018", 22.68, 22.1, 22.83, 22.08];
...
// const arrInput = [26.83, 27.1, 27.12, 26.64]; // 09.05.2019
...
model
  ...
  .then(() => {
    ...
    console.log(`Maxima:     R$ ${Number(output[2]).toFixed(2)}`);
    console.log(`Minima:     R$ ${Number(output[3]).toFixed(2)}`);
  });
